...
if iscell(type)
    ...
    if strcmp(type, 'fast')
        ...
        if exist('tolerance', 'var')
            % if the function is called from a macro (tolerance already known)
            % get the list of simplified polygon using the tolerance
            polygons = polygonsSimplify(obj, 'off', tolerance);
            macro = 'on';
        else
            % if the function isn't called from a macro (tolerance unknown)
            % call the polygonsSimplufy function and get both the list
            % of simplified polygons and the tolerance used during the
            % simplification
            [polygons, tolerance] = polygonsSimplify(obj, 'off');
            macro = 'off';
            if strcmp(tolerance, '?')
                return;
            end
        ...
        obj.model.usedProcess{end+1} = ['polygonsAlign : type = ' type ' ; axis = ' axis ' ; tolerance = ' num2str(tolerance)];
    else 
